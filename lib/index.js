    ...
    posts: (callbackFunc, _locale = 'ko-KR') => {
        ...
        try {
            ...
            _app.bands.forEach(async (v, i) => {
                ...
                let response = await axios.get(originalURL + '/v2/band/posts' + query);
                if(error(response.data)) {
                ...
                _app.posts[v.band_key] = {};
                _app.posts[v.band_key].items = response.data.result_data.items;
                _app.posts[v.band_key].paging = response.data.result_data.paging;
            
                callbackFunc(undefined, v.name, v.band_key);
                queue.dequeues();
            });
